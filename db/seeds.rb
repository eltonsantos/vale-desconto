...
Rails.logger.debug("Criando usuário padrão")
users = User.create!([{ email: "admin@admin.com", password: 123456 }])

Rails.logger.debug("Criando produtos")
products = Product.create!([
  { title: "Produto 1", description: "Descrição do Produto 1", price: 10.0 },
])

Rails.logger.debug("Criando descontos")
Discount.create([
  {
    name: "Biscoito integral cereale Cacau 150g",
    description: "Biscoito Bauduco Cereale Cacau, Aveia e Mel Integrais é uma Fonte de fibras e com cereais integrais.",
    discount_type: 0,
...
Rails.logger.debug("Salvando imagens")
Discount.all.find_each do |discount|
